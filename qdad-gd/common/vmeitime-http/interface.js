...
export default {
	...
	request(options) {
		...
		var customPort = options.url
		options.url = options.baseUrl + options.url
		options.method = options.method || this.config.method
		var parm = options.data || {}
		var common = {
			token: store.state.token
		}
		options.data = Object.assign({}, parm, common);
		if (customPort !== '/api/resident/list' && customPort !== '/im/query/history/conversation') {
			uni.showLoading({
				title: '',
				mask: true
			});
...
function _reslog(res) {
	...
	console.log('**********************'+JSON.stringify(_statusCode))
	if (process.env.NODE_ENV === 'development') {
